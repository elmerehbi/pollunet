...
mask = Cropping2D(o)(mask)

x = sar

x = unet_layers(x,depth)

x = concatenate([x,mask],axis=1)

for i in range(nb_conv_out):
    x = Conv2D(nbClass,kernel,padding='valid',data_format='channels_first')(x)
...
###########################
    ...
    # Train mask
    train_mask=f["masks/train/"+fl][:][:,o:-o,o:-o,:].reshape((-1,size_out*size_out,nbClass))
    # Input mask
    input_mask = f["masks/train/"+fl][...,[1,3]]
      

    if os.path.isfile(weight_dir+fl):
        unet.load_weights(weight_dir+fl)

    unet.fit([train_nrcs,input_mask],train_mask,shuffle='batch',verbose=1,batch_size=batch_size,epochs=epochs,sample_weight=w)
