...
def unet_layers(x,depth,freeze):
    fr = depth < freeze
    if depth == 0:
        for i in range(nb_conv-1):
            x=Conv2D(channels(depth),kernel,padding='valid',activation=activation,kernel_initializer='he_normal',data_format='channels_first',trainable=fr)(x)
        x=Conv2D(channels(depth),kernel,padding='valid',activation=activation,kernel_initializer='he_normal',data_format='channels_first',trainable=fr)(x)
    else:
    ...
    if depth>0:
        ...
        y=unet_layers(y,depth-1,freeze)
...
unet.load_weights(weight_dir+'deep5.py')
print unet.summary()
exit()
# print size_out

unet.compile(optimizer='adadelta', loss='categorical_crossentropy',
