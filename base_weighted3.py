...
with h.File(hdf,"a") as f:
    ...
    if not f.__contains__("weights/"+fl):
        ...
        f.require_dataset("masks/testing_images/"+fl,(2471,size_out*size_out,nbClass),maxshape=(None,size_out*size_out,nbClass),dtype='f4')
        ...
        a%=4
        a=a.reshape(-1,b)
        f.require_dataset("masks/training_images/"+fl,(1700,size_out*size_out,nbClass),maxshape=(None,size_out*size_out,nbClass),dtype='f4')
        ...
        w=w.reshape(a,-1)
        del q
        f.create_dataset('weights/'+fl,(10629,b))
        f["weights/"+fl][:]=w
    ...
    f.require_dataset("results/testing_images/"+fl,shape=(2471,420,420,3),dtype='f4',exact=False)
    f["results/testing_images/"+fl][:]=v.reshape(-1,420,420,3)
    f.require_dataset("segmentation/testing_images/"+fl,shape=(2471,420,420),dtype='i8',exact=False)
    f["segmentation/testing_images/"+fl][:]=m.to_classes(v).reshape(-1,420,420)

    test_nrcs=f["test/Nrcs/training_images/"]
    w=eddynet.predict(test_nrcs)
